...
func input(stages []string, stageA, stageB, stageC *tview.List, grid *tview.Grid, finderFocus tview.Primitive, event *tcell.EventKey) *tcell.EventKey {
	...
	case tcell.KeyCtrlA:
		...
		addTaskForm := tview.NewForm().
			AddInputField("Task heading: ", "", 50, nil, nil).
			AddInputField("Task description: ", "", 50, nil, nil).
...
func run(path string, stages []string) (err error) {
	...
	// declare heads
	header := newHeadPrimitive("Personal Kanban")
	footer := newHeadPrimitive("Ctrl+a - Add | Ctrl+b - Del | 'd' - right column | 'a' - left column | Esc - quit")
